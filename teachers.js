    ...
    try {
            ...
            <div style="overflow-x:auto;">
                <table>
                    ...
                    <tbody id="teachers-directory-tbody">
                        <tr><td colspan="6" style="text-align:center; padding:15px; font-weight:bold;">⏳ جاري جرد الدليل السحابي لمدرستك...</td></tr>
                    </tbody>
                </table>
            </div>
        </div>`;
        ...
        snap.forEach(doc => {
            ...
            html += `<tr style="border-bottom:1px solid #eee;">
                ...
                <td>${d.department || '<span style="color:#aaa;">—</span>'}</td>
                <td>${d.email ? `<a href="mailto:${d.email}" style="color:var(--sky);">${d.email}</a>` : '<span style="color:#aaa;">—</span>'}</td>
                <td style="text-align:center;">${d.phone ? `<a href="tel:${d.phone}" style="color:var(--sky);">${d.phone}</a>` : '<span style="color:#aaa;">—</span>'}</td>
            </tr>`;
        });

        document.getElementById('teachers-directory-tbody').innerHTML = html || '<tr><td colspan="6" style="text-align:center; padding:15px;">💡 لا يوجد موظفين مقيدين بهذه المدرسة.</td></tr>';
    } catch(e) { 
