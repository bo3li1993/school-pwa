// ⚠️ موديل رصد وتحليل حالات الغياب المتكرر والتراكمي للطلاب
...
export async function initRepeatModule() {
    ...
    try {
        ...
        <div class="card" style="border-top: 5px solid var(--danger-color); text-align: right; background:#fff; padding:20px; border-radius:12px;">
            <h2><i class="bi bi-exclamation-octagon-fill" style="color:var(--danger-color);"></i> نظام الرصد والمتابعة التراكمية للغياب المتكرر</h2>
            <p style="font-size:12px; color:#666; margin-bottom:20px; font-weight:bold;">
                📊 يقوم المحرك بمسح كشوف الرصد المرفوعة وحصر الطلاب الذين تخطى غيابهم يومين أو أكثر لإصدار إجراء إداري فوري.
            </p>
                ...
                <table>
                    <thead>
                        <tr style="background:#fff0f0; color:var(--danger-color);">
                            ...
                            <th style="text-align:center;">إجمالي أيام الغياب المرصودة</th>
...
async function calculateRepeatAbsencesLive() {
    ...
    try {
        ...
        // تصفية وحصر الحالات التي تكرر غيابها يومين أو أكثر
        ...
        filtered.forEach(s => {
            ...
            if (s.count >= 5) lawBadge = `<span class="badge danger" style="background:#active; font-weight:900;">🚨 استدعاء ولي أمر فوري</span>`;
        ...
        tbody.innerHTML = html || '<tr><td colspan="4" style="text-align:center; color:var(--success-color); padding:15px; font-weight:bold;">🥇 السجلات نظيفة بالكامل، لا توجد حالات غياب متكرر هذا الأسبوع.</td></tr>';
